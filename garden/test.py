...
creator = YOLODetector('/home/pi/garden/best-l-fp16.tflite','/home/pi/garden/label.txt',0.4,0.65)
...
while True:
	...
	if state == 1:
		...
		if coords == [] or len(coords) == 0:
			...
			cv2.putText(frame, "Radar(FAR)", (max(int(coords[0]),0), max(int(coords[1]-55),0)), cv2.FONT_ITALIC, 0.5, (0,100,255), 1, 0)
			cv2.putText(frame, "Helicopter", (max(int(coords[0]),0), max(int(coords[1]-25),0)), cv2.FONT_ITALIC, 0.5, (0,100,255), 1, 0)
			cv2.putText(frame, "GUN       ", (max(int(coords[0]),0), max(int(coords[1]-40),0)), cv2.FONT_ITALIC, 0.5, (0,100,255), 1, 0)
			cv2.putText(frame, "Missiles  ", (max(int(coords[0]),0), max(int(coords[1]-10),0)), cv2.FONT_ITALIC, 0.5, (0,100,255), 1, 0)
